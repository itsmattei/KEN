...
class Kernel_injection:
    ...
    def inject_attention_params(self):
        parameters_to_inject = [param for param in self.state_dict_W0 if re.search("[0-9]", param) != None]

        # Set non-injectable parameters to trained values
        for param in set(self.state_dict_W0).difference(parameters_to_inject):
            self.state_dict_W0[param] = self.state_dict_trained[param]

        # Load the state dict with non-injectable parameters set
        self.model_W0.load_state_dict(self.state_dict_W0)

        # Inject parameters in parallel
        for index in tqdm(range(0, len(parameters_to_inject))):
            try:
                injected_state_dict = self.injection_values(parameters_to_inject[index])
            except:
