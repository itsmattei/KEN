...
class Kernel_injection:
    ...
    def inject_all_parameters(self):
        params = [param for param in self.state_dict_trained][:-1]

        for index in tqdm_notebook(range(0, len(params))):
            injected_state_dict = self.injection_values(params[index])

        self.model_trained.load_state_dict(injected_state_dict)
